...
const paramsSchema = {
  ...
  confirmationId: z
    .string()
    .describe(
      'This confirmationId must be obtained from confirm_uninstall_wordpress tool after explicit user confirmation. If an invalid or mismatched confirmationId is provided, the operation will be rejected.'
    ),
}

const paramsZodObject = z.object(paramsSchema)

export const uninstallWordPressTool: ForgeToolDefinition<typeof paramsSchema> = {
  ...
  handler: async (params, forgeApiKey) => {
    try {
      ...
      const confirmation = validateConfirmation(
        uninstallWordPressConfirmationStore,
        confirmationId,
