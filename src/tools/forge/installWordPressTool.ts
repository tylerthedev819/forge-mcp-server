...
const paramsSchema = {
  ...
  clearExistingApp: z
    ...
    .default(true)
    .describe(
      'Whether to automatically clear any existing application before installing WordPress. Defaults to true. This allows WordPress to be installed on sites that already have an app deployed (like the default site page).'
    ),
  confirmationId: z
    .string()
    .describe(
      'This confirmationId must be obtained from confirm_install_wordpress tool after explicit user confirmation. If an invalid or mismatched confirmationId is provided, the operation will be rejected.'
    ),
}
